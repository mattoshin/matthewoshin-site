...
export default function BeaconLanding() {
  return (
    <div className="relative">
      {/* nav */}
      <header className="sticky top-12 z-20 border-b border-[var(--fc-border)] bg-[var(--fc-bg)]/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
          <Wordmark />
          <nav className="hidden items-center gap-7 text-[13px] text-[var(--fc-muted)] md:flex">
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} className="transition-colors hover:text-[var(--fc-ink)]">
                {l.label}
              </a>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <Link href={DASH} className="hidden text-[13px] text-[var(--fc-muted)] transition-colors hover:text-[var(--fc-ink)] sm:block">
              Log in
            </Link>
            <Link
              href={DASH}
              className="rounded-lg bg-[var(--fc-ink)] px-4 py-2 text-[13px] font-medium text-white transition-colors hover:bg-black"
            >
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-5 py-12 sm:px-8 sm:py-20 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-[var(--fc-accent)]/25 bg-[var(--fc-accent-wash)] px-3 py-1 text-[12px] font-medium text-[var(--fc-accent)]">
            <Icon name="sparkles" size={13} /> The AI intelligence layer for IR, PR, and capital markets
          </div>
          <h1 className="mt-5 text-4xl font-semibold leading-[1.08] tracking-tight text-[var(--fc-ink)] sm:text-[56px]">
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            ...
            <a href="#modules" className="inline-flex items-center gap-2 rounded-lg border border-[var(--fc-border-strong)] bg-[var(--fc-card)] px-6 py-3 text-[14px] font-medium text-[var(--fc-ink)] transition-colors hover:bg-[var(--fc-surface-2)]">
              Explore modules
            </a>
          </div>
        ...
        <div className="grid grid-cols-2 overflow-hidden rounded-[12px] border border-[var(--fc-border)] sm:grid-cols-4">
          {[
            ...
            <div key={s.l} className={cx("bg-[var(--fc-card)] px-6 py-7 text-center", i < 3 && "sm:border-r", "border-[var(--fc-border)]")}>
              ...
              <div className="mt-1 text-[11px] uppercase tracking-wider text-[var(--fc-faint)]">{s.l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
        <h2 className="max-w-2xl text-2xl font-semibold tracking-tight text-[var(--fc-ink)] sm:text-3xl">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {BEACON_MODULES.map((m) => (
            ...
            >
              ...
              <h3 className="mt-3 text-[14px] font-semibold text-[var(--fc-ink)]">{m.name}</h3>
              <p className="mt-1.5 text-[12.5px] leading-relaxed text-[var(--fc-muted)]">{m.blurb}</p>
              <span className="mt-3 inline-flex items-center gap-1 text-[12px] font-medium text-[var(--fc-accent)] opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <div className="mt-9 grid gap-4 md:grid-cols-3">
          {BEACON_STEPS.map((s) => (
            <div key={s.n} className="rounded-[10px] border border-[var(--fc-border)] bg-[var(--fc-card)] p-6">
              ...
              <p className="mt-2 text-[13.5px] leading-relaxed text-[var(--fc-muted)]">{s.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="stack" className="mx-auto max-w-6xl px-5 pb-8 sm:px-8">
        <div className="rounded-[12px] border border-[var(--fc-border)] bg-[var(--fc-card)] p-6 sm:p-8">
          <div className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.2em] text-[var(--fc-muted)]">
            <Icon name="layers" size={14} /> Built with
          </div>
          <p className="mt-2 max-w-2xl text-[13.5px] leading-relaxed text-[var(--fc-muted)]">
            ...
            {BEACON_STACK.map((g) => (
              <div key={g.group}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-semibold uppercase tracking-wider text-[var(--fc-ink)]">{g.group}</span>
                </div>
                <ul className="mt-2.5 space-y-1.5">
                  {g.items.map((it) => (
                    <li key={it} className="text-[12.5px] leading-snug text-[var(--fc-muted)]">{it}</li>
                  ))}
                </ul>
              </div>
            ))}
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24">
        <div className="rounded-[16px] border border-[var(--fc-border)] bg-[var(--fc-ink)] p-10 text-center sm:p-14">
          ...
          <Link
            href={DASH}
            className="mt-8 inline-flex items-center gap-2 rounded-lg bg-white px-7 py-3.5 text-[14px] font-semibold text-[var(--fc-ink)] transition-transform hover:scale-[1.02]"
          >
...
function HeroPreview() {
  return (
    <div className="justify-self-center">
      <div className="w-full max-w-md overflow-hidden rounded-[14px] border border-[var(--fc-border)] bg-[var(--fc-card)] shadow-[0_24px_48px_-12px_rgba(12,14,19,0.12)]">
        {/* window chrome */}
        <div className="flex items-center gap-2 border-b border-[var(--fc-border)] bg-[var(--fc-recessed)] px-3 py-2.5">
          <Wordmark size="sm" />
          <span className="ml-auto flex items-center gap-1 rounded-full border border-[var(--fc-up)]/25 bg-[#ecfdf3] px-2 py-0.5 text-[10px] font-medium text-[var(--fc-up)]">
            <span className="h-1.5 w-1.5 rounded-full bg-[var(--fc-up)]" /> Live
          </span>
        </div>
        <div className="space-y-3 p-4">
          {/* AI brief */}
          <div className="rounded-r-[10px] border border-l-2 border-[var(--fc-border)] border-l-[var(--fc-accent)] bg-[var(--fc-card)] p-3">
            <div className="mb-1.5 flex items-center gap-1.5">
              <span className="inline-flex items-center gap-1 rounded-full border border-[var(--fc-accent)]/25 bg-[var(--fc-accent-wash)] px-2 py-0.5 text-[10px] font-medium text-[var(--fc-accent)]">
                <Icon name="sparkles" size={10} /> Morning brief
              </span>
            </div>
            <p className="text-[12px] leading-relaxed text-[var(--fc-ink-2)]">
              Three clients report this week, led by Quanta Labs Thursday. Software multiples up 6% on cooling inflation.
            </p>
          </div>
          {/* stat tiles */}
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg border border-[var(--fc-border)] bg-[var(--fc-recessed)] p-2.5">
              <div className="text-[10px] uppercase tracking-wide text-[var(--fc-faint)]">Consensus</div>
              <div className="mt-1 font-mono text-[15px] font-semibold tabular-nums text-[var(--fc-ink)]">$158 PT</div>
            </div>
            <div className="rounded-lg border border-[var(--fc-border)] bg-[var(--fc-recessed)] p-2.5">
              <div className="text-[10px] uppercase tracking-wide text-[var(--fc-faint)]">NRR</div>
              <div className="mt-1 font-mono text-[15px] font-semibold tabular-nums text-[var(--fc-ink)]">119%</div>
            </div>
          </div>
          {/* ticker row */}
          <div className="flex items-center justify-between rounded-lg border border-[var(--fc-border)] px-3 py-2">
            <span className="font-mono text-[13px] font-semibold text-[var(--fc-accent)]">QNTA</span>
            <span className="font-mono text-[13px] tabular-nums text-[var(--fc-ink)]">$142.50</span>
            <span className="font-mono text-[12px] font-medium tabular-nums text-[var(--fc-up)]">▲ +1.82%</span>
          </div>
        </div>
      </div>
    </div>
