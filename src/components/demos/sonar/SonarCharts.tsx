      ...
      {items.map((i) => (
        <div key={i.name}>
          <div className="flex items-center justify-between text-xs">
            <span className="truncate text-[var(--s-text)]">{i.name}</span>
            <span className="ml-3 shrink-0 font-mono text-[var(--s-muted)]">{format(i.count)}</span>
          </div>
          <div className="mt-1 h-1.5 overflow-hidden rounded-full" style={{ background: "var(--s-panel-2)" }}>
  ...
  return (
    <div className="flex items-center gap-5">
      <svg width={size} height={size} viewBox="0 0 100 100" className="shrink-0">
        <circle cx="50" cy="50" r={r} fill="none" stroke="var(--s-panel-2)" strokeWidth="11" />
        ...
        {centerSub && (
          <text x="50" y="62" textAnchor="middle" style={{ fontSize: 7, fill: "var(--s-faint)", letterSpacing: 1 }}>
            {centerSub}
          </text>
        )}
      </svg>
      <div className="space-y-2">
        {segments.map((seg) => (
          <div key={seg.label} className="flex items-center gap-2 text-sm">
            ...
            <span className="text-[var(--s-text)]">{seg.label}</span>
            <span className="font-mono text-xs text-[var(--s-faint)]">{seg.value.toLocaleString()}{unit}</span>
          </div>
        ))}
      </div>
    </div>
...
/** Stacked vertical bars for multiple series (e.g. daily cost by model). */
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4">
        {keys.map((k) => (
          <div key={k.key} className="flex items-center gap-1.5 text-xs text-[var(--s-muted)]">
