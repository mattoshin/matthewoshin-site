...
function PortfolioCard({ item, eager }: { item: PortfolioItem; eager: boolean }) {
  ...
  const header = (
    <h2 className="min-w-0 font-serif text-2xl font-semibold leading-snug text-ink-heading">
      {item.name}
    </h2>
  );
  ...
  if (item.caseHref) {
    ...
    return (
      ...
      >
        {media}
        <h2 className="min-w-0 font-serif text-2xl font-semibold leading-snug text-ink-heading transition-colors group-hover:text-bio-cyan">
          {item.name}
        </h2>
        <p className="mt-3 text-sm text-ink-body sm:text-base">{item.hook}</p>
